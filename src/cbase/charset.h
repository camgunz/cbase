...
bool charset_convert(Buffer *in, const char *from, const char *to,
                                                   Buffer *out,
                                                   Status *status);

static inline bool charset_localize(Buffer *in, Buffer *out, Status *status) {
    return charset_convert(in, "UTF-8", "wchar_t", out, status);
}

static inline bool charset_unlocalize(Buffer *in, Buffer *out,
                                                  Status *status) {
